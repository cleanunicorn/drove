...
def test_save_and_load_roundtrip(tmp_path: Path) -> None:
    model = fake_model(tmp_path)
    cfg = ModelConfig(ctx_size=4096, n_gpu_layers=32, flash_attn=True)
...
def test_to_llama_args(tmp_path: Path) -> None:
    cfg = ModelConfig(ctx_size=8192, n_gpu_layers=-1, flash_attn=True, temp=0.7)
...
def test_set_model_config_key_bool(tmp_path: Path) -> None:
    model = fake_model(tmp_path)
    updated = set_model_config_key(model, "flash_attn", "true")
    assert updated.flash_attn is True
